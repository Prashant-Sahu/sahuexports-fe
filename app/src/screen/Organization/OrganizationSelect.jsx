...
const OrganizationSelect = () => {
  ...
  const data = [
    { label: 'Microssoft', value: 'Microssoft' },
    { label: 'Moserbear', value: 'Moserbear' },
    { label: 'Sun Micro', value: 'Sun Micro' },
  ];

  const [value, setValue] = useState(null);

  const databrunch = [
    { label: 'Bruanch-1', value: 'Bruanch-1' },
    { label: 'Bruanch-2', value: 'Bruanch-2' },
    { label: 'Bruanch-3', value: 'Bruanch-3' },
  ];
  ...
  const handleDashboard = () => 
    {
      ...
      if(valuebranch==null || valuebranch==undefined || valuebranch=='')
        {
          showMessage({
            description: "Warning message",
    ...
    const validateUser = async () =>
      {
        ...
        console.log(`Login id ${companyCode} Req Body ${JSON.stringify(body)}`);
          ...
          await axios.post("http://172.27.176.1:8080"+"/api/user/v1/login",body).then(async res=>
          {
            console.log("Called Validate User Function under");
              if(res.data=="User authenticated successfully")
              {
                ...
                console.log(`Full Name is Before Set: ${userInfo.usercreated.toString()}`);
                await setIsloading(false);
                console.log(`User Validated Successfully Data received : ${JSON.stringify(userInfo)}`);
   ...
   <View style={styles.container}>
         {/* <ImageBackground source={require('./src/assets/Images/Background/MLoginBGWithoutLogoNONO.png')} blurRadius={0} resizeMode="cover" style={styles.image}> */}
           <View style={styles.textContainer}>
             {isLoading ? (<ActivityIndicator size="large" color={colors.greenBar} />) : null}
           </View>
           <View style={{flex:4,flexDirection:'column',alignContent:'center',alignItems:'center'}}>
